...
<?php
	...
	if($member_user  !=  NULL)
	   {
			include('../config.php');
			$sql_show  	=	"select  *  from  members where member_user =  '".$member_user."' ";
			$result_show  	=	mysql_query($sql_show);
			while($show  	=	mysql_fetch_assoc($result_show))
					{
?>		   
		   
  <form method="post" name="frm-add" id="frm-add" autocomplete="off" action="<?=$_SERVER['PHP_SELF'];?>?link=uri_user">
...
if($_SERVER['REQUEST_METHOD']  ==  'POST')
  {
	  ...
	  $sql_edit  =  "update  members  set   member_pass  =  '".$member_pass."'  ,  member_name  =  '".$member_name."'  ,  member_sex  =  '".$member_sex."'  ,  member_address  =  '".$member_address."'  ,  member_tel  =  '".$member_tel."' where member_user  =  '".$member_user."'   ";
	  $result_edit  =  mysql_query($sql_edit);
	  if($result_edit  == true)
		{
			echo  "<script>alert('Edit succeed full');window.location='edit-member.php?link=uri_user&say_user=".$member_user."';</script>";  
		}		
	mysql_close($config);
  }
?>
