...
<?php
		include('../config.php');
		$sql_show	=	"select  count(dt.pro_id) as num_qty ,  sum(dt.det_price) as total_price , bk.book_id as new_id ,  bk.* , dt. *  from  books as bk  left join detail_books as dt  on bk.book_id  =  dt.book_id where  bk.book_date  like '".$find_date."%'  and  bk.member_user = '".$_SESSION['se_id']."' group by dt.book_id   order by  bk.book_date , bk.book_time asc ";
...
&nbsp;น.</span></td>
		...
		?>
    	  <tr>
    	    <td height="100" colspan="5" align="center"><input type="button" name="button" id="button" value=" ย้อนหลัง " class="btn"  onClick="window.location='show-resume.php?say=1&say_date=<?=$_GET['say_date'];?>'"></td>
   	    </tr>
