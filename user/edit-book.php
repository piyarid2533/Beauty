...
<div class="container">
	...
	$say_date	=	$_GET['say_date'];
    if($say_date  !=  NULL)
		{
			include('../config.php');

			$sql_show	=	"select * from books  where  book_date  =  '".$say_date."' ";
  ...
  <div class="show">
	  <table width="1000" border="0" cellpediting="0" cellspacing="0">
	    <tr>
        ...
        <input name="hid_date" type="hidden" id="hid_date" value="<?=$_GET['say_date'];?>">
        แก้ไข ตารางจองเวลา ประจำวันที่ &nbsp;&nbsp;<?=date('d - m - Y', strtotime($find_date));?></h4></th>
        </tr>
	    <tr >
	    ...
	    <tr>
	      ...
	      <td height="80" colspan="5" align="left"><img src="../img/cross.png" width="16" height="16"  alt=""/> = <span  class="cl-pink">ไม่สามารถจองได้</span></td>
	      <td height="80" colspan="2" align="left"><input type="button" name="button" id="button" value="ย้อนกลับ" class="btn" onClick="window.location='show-book.php?link=uri_book';"></td>
	      <td height="80" colspan="2" align="left"><input type="submit" name="submit" id="submit" value="แก้ไขการจองเวลา" class="btn" ></td>
        </tr>
      </table>
  </div>
...
if($_SERVER['REQUEST_METHOD']  ==  'POST'  and  $_REQUEST['submit'])
	{
		$say_id	=	$_POST['hid_id'];
		$say_date	=	$_POST['hid_date'];
		$book_time	=	$_POST['radio'];
		
		if($book_time  ==  NULL)
		  {
				echo  "<script>alert('กรุณาเปลี่ยนแปลงข้อมูล');window.location='edit-book.php?link=uri_book&say_id=".$say_id."&say_date=".$say_date."';</script>";  
		  }
		else
		  {
			  	include('../config.php');
				
				$sql_edit	=	"update  books set  book_time = '".$book_time."'  where  book_id  =  '".$say_id."'  ";
				$result_edit   =  mysql_query($sql_edit);
				if($result_edit  == true)
				  {
					  echo  "<script>alert('edit succeed full');window.location='edit-book.php?link=uri_book&say_id=".$say_id."&say_date=".$say_date."';</script>";  
				  }
				mysql_close($config);
		  }
	}
