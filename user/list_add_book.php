 ...
 if($_REQUEST['btn_search'])
   {
	   ...
	   if($check_rows  ==  false)
	     {
			 echo  "<script>alert('ไม่พบข้อมูล');window.location='list_add_book.php?link=uri_det&say_id=".$book_id."';</script>";
		 }
	   else
	     {
 ?>   
	...
	<form action="<?=$_SERVER['PHP_SELF'];?>?link=uri_det" method="post" name="frm_add" id="frm_add">
    <div class="show">
	  <table width="1000" border="0" cellspacing="0" cellpadding="0">
	    <tr>
          ...
          <input name="say_id2" type="hidden" id="say_id2" value="<?=$_GET['say_id'];?>"></th>
	    ...
	    <tr>
	      ...
	      <td width="120" height="70" align="left"><input type="button" name="button2" id="button2" value="ย้อนกลับ" class="btn"  onClick="window.location='add-det.php';"></td>
	      <td width="120" height="70" align="left"><input type="button" name="button" id="button" value="New search" class="btn"  onClick="window.location='list_add_book.php?link=uri_det&say_id=<?=$_GET['say_id'];?>';"></td>
...
if($_SERVER['REQUEST_METHOD']  ==  'POST'  and  $_REQUEST['btn_add'])
  {
		...
		echo  "<script>alert('Save Succeed full');window.location='list_add_book.php?link=uri_det&say_id=".$say_id."';</script>";
  }
